...
extension LoRAZoo {
  ...
  private static func internalRequestNetworkPayload() {
    let request = URLRequest(url: URL(string: "https://models.drawthings.ai/loras.json")!)
    let task = URLSession.shared.downloadTask(with: request) { url, response, error in
      ...
      try? fileManager.removeItem(at: networkUrl.appendingPathComponent("loras.json"))
      try? fileManager.moveItem(at: url, to: networkUrl.appendingPathComponent("loras.json"))
    ...
    let sha256Request = URLRequest(
      url: URL(string: "https://models.drawthings.ai/loras_sha256.json")!)
    let sha256Task = URLSession.shared.downloadTask(with: sha256Request) { url, response, error in
      ...
      let networkUrl = cacheUrl.appendingPathComponent("net")
      try? fileManager.createDirectory(at: networkUrl, withIntermediateDirectories: true)
      try? fileManager.removeItem(at: networkUrl.appendingPathComponent("loras_sha256.json"))
      try? fileManager.moveItem(
        at: url, to: networkUrl.appendingPathComponent("loras_sha256.json"))
      mergeFileSHA256()
    }
    sha256Task.resume()
  }
  ...
  private static func communityFromDisk() -> [Specification] {
    ...
    let urls = fileManager.urls(for: .cachesDirectory, in: .userDomainMask)
    let cacheUrl = urls.first!
    let modelsUrl = cacheUrl.appendingPathComponent("net").appendingPathComponent("loras.json")
  ...
  public static var community: [Specification] = {
    let availableModels = Set(
      availableSpecifications.compactMap {
        ($0.deprecated == true && isBuiltinLoRA($0.file)) || !isModelDownloaded($0)
          ? nil : $0.file
      })
