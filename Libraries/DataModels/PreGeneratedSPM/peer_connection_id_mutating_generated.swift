...
extension PeerConnectionId: FlatBuffersEncodable {
  public func to(flatBufferBuilder: inout FlatBufferBuilder) -> Offset {
    let __id = flatBufferBuilder.create(string: self.id)
    let __updatedAt = self.updatedAt.map { flatBufferBuilder.create(string: $0) } ?? Offset()
    let start = zzz_DflatGen_PeerConnectionId.startPeerConnectionId(&flatBufferBuilder)
    zzz_DflatGen_PeerConnectionId.add(id: __id, &flatBufferBuilder)
    zzz_DflatGen_PeerConnectionId.add(updatedAt: __updatedAt, &flatBufferBuilder)
    return zzz_DflatGen_PeerConnectionId.endPeerConnectionId(&flatBufferBuilder, start: start)
  }
}

extension Optional where Wrapped == PeerConnectionId {
...
public final class PeerConnectionIdChangeRequest: Dflat.ChangeRequest {
  private var _o: PeerConnectionId?
  public typealias Element = PeerConnectionId
  ...
  private init(type _type: ChangeRequestType, _ _o: PeerConnectionId) {
    self._o = _o
    self._type = _type
    _rowid = _o._rowid
    id = _o.id
    updatedAt = _o.updatedAt
  }
  public static func changeRequest(_ o: PeerConnectionId) -> PeerConnectionIdChangeRequest? {
    ...
    let u = transactionContext.objectRepository.object(
      transactionContext.connection, ofType: PeerConnectionId.self, for: key)
    return u.map { PeerConnectionIdChangeRequest(type: .update, $0) }
  }
  public static func upsertRequest(_ o: PeerConnectionId) -> PeerConnectionIdChangeRequest {
    let transactionContext = SQLiteTransactionContext.current!
    let key: SQLiteObjectKey = o._rowid >= 0 ? .rowid(o._rowid) : .primaryKey([o.id])
    guard
      let u = transactionContext.objectRepository.object(
    ...
    let changeRequest = PeerConnectionIdChangeRequest(type: .update, o)
    changeRequest._o = u
    changeRequest._rowid = u._rowid
    return changeRequest
  }
  public static func creationRequest(_ o: PeerConnectionId) -> PeerConnectionIdChangeRequest {
    let creationRequest = PeerConnectionIdChangeRequest(type: .creation, o)
    creationRequest._rowid = -1
    return creationRequest
  }
  public static func creationRequest() -> PeerConnectionIdChangeRequest {
    return PeerConnectionIdChangeRequest(type: .creation)
  }
  public static func deletionRequest(_ o: PeerConnectionId) -> PeerConnectionIdChangeRequest? {
    let transactionContext = SQLiteTransactionContext.current!
    let key: SQLiteObjectKey = o._rowid >= 0 ? .rowid(o._rowid) : .primaryKey([o.id])
    let u = transactionContext.objectRepository.object(
      transactionContext.connection, ofType: PeerConnectionId.self, for: key)
    return u.map { PeerConnectionIdChangeRequest(type: .deletion, $0) }
  }
  var _atom: PeerConnectionId {
    let atom = PeerConnectionId(id: id, updatedAt: updatedAt)
    atom._rowid = _rowid
    return atom
  }
    ...
    case .creation:
      guard
        let insert = toolbox.connection.prepareStaticStatement(
          "INSERT INTO peerconnectionid_v1 (__pk0, p) VALUES (?1, ?2)")
      else { return nil }
    ...
    case .update:
      ...
      guard
        let update = toolbox.connection.prepareStaticStatement(
          "REPLACE INTO peerconnectionid_v1 (__pk0, p, rowid) VALUES (?1, ?2, ?3)")
      else { return nil }
    ...
    case .deletion:
      guard
        let deletion = toolbox.connection.prepareStaticStatement(
          "DELETE FROM peerconnectionid_v1 WHERE rowid=?1")
      else { return nil }
