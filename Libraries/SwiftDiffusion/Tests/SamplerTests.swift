...
final class SamplerTests: XCTestCase {
  func testOrdinaryModelsPreserveWeightedAddAndBroadcasting() {
    ...
    graph.withNoGrad {
      ...
      let expected = Functional.add(left: sample, right: noise, leftScalar: 0.7, rightScalar: 0.4)
        .rawValue.toCPU()
      let added = sampleAdd(sample, noise: noise, scale: (sample: 0.7, noise: 0.4), version: .v1)
        .rawValue.toCPU()
      var calls = 0
      let combined = sampleScaledAdd(sample, noise, sigma: (now: 0.8, next: 0.2), version: .v1) {
        sigma in
        calls += 1
  ...
  func testH3CallbackMapsBothSigmasAndDoesNotModifyOperands() {
    ...
    graph.withNoGrad {
      ...
      let result = sampleScaledAdd(left, right, sigma: (now: 0.8, next: 0.3), version: .minimaxH3) {
        sigma in
        sigmas.append(sigma)
      ...
      let aliased = sampleScaledAdd(left, left, sigma: (now: 0.6, next: 0), version: .minimaxH3) {
        sigma in
        (sigma.now, 0)
      }.rawValue.toCPU()
  ...
  func testH3MultistepCleanPredictionAndLogSNR() {
    ...
    graph.withNoGrad {
      ...
      for history: (previous: Float, now: Float, next: Float) in [
        ...
        let velocity = sampleScaledAdd(rawVelocity, rawVelocity, sigma: step, version: .minimaxH3) {
          sigma in
          let delta = step.now - step.next
          return (delta > 0 ? (sigma.now - sigma.next) / delta : 0, 0)
        }
        let denoised = sampleScaledAdd(sample, velocity, sigma: step, version: .minimaxH3) {
          sigma in
        ...
        let result = sampleScaledAdd(sample, combined, sigma: step, version: .minimaxH3) { sigma in
  ...
  func testH3UniPCPredictorAndCorrectorCoefficients() {
    ...
    graph.withNoGrad {
      ...
      for schedule in [[1.0, 0.8, 0.4], [0.91, 0.5, 0.05]] {
        ...
        let base = sampleScaledAdd(x, m0, sigma: step, version: .minimaxH3) { sigma in
          let w = Double(sigma.next) / Double(sigma.now)
          return (Float(w), Float(1 - w))
        }
        let predictor = sampleScaledAdd(base, d1, sigma: step, version: .minimaxH3) { sigma in
          (1, Float((1 - Double(sigma.next) / Double(sigma.now)) / 2))
        }.rawValue.toCPU()
        let corrector = sampleScaledAdd(base, dc, sigma: step, version: .minimaxH3) { sigma in
      ...
      for step: (now: Float, next: Float) in [(0.8, 0.4), (0.1, 0), (0, 0)] {
        ...
        for gamma: Float in [0, 0.3, 1] {
          ...
          let adjusted = sampleScaledAdd(
            predicted, denoised, sigma: noiseSigma, version: .minimaxH3
          ) { sigma in
            (1, sigma.next - sigma.now)
          }
          let result = sampleScaledAdd(adjusted, freshNoise, sigma: noiseSigma, version: .minimaxH3)
          { sigma in
    ...
    graph.withNoGrad {
      ...
      for step: (now: Float, next: Float) in [(1, 0.9), (0.8, 0.4), (0.5, 0.01), (1, 0.99)] {
        ...
        let sample = sampleAdd(
          clean, noise: oldNoise, scale: (sample: 1 - step.now, noise: step.now),
          version: .minimaxH3)
        let brownian = sampleScaledAdd(basis[0], basis[0], sigma: step, version: .minimaxH3) {
          sigma in (sigma.now.squareRoot(), 0)
        }
        let leftW = sampleScaledAdd(brownian, basis[1], sigma: first, version: .minimaxH3) {
          sigma in
        ...
        var x2 = sampleScaledAdd(sample, clean, sigma: first, version: .minimaxH3) { sigma in
          let ratio = Double(sigma.next) / Double(sigma.now)
          return (Float(ratio * ratio), Float(1 - ratio * ratio + Double(sigma.next) * (ratio - 1)))
        }
        x2 = sampleScaledAdd(x2, rightW, sigma: first, version: .minimaxH3) { sigma in
          (
            1,
            Float(
        ...
        let velocity = sampleScaledAdd(x2, clean2, sigma: second, version: .minimaxH3) { sigma in
          let scale = (sigma.now - sigma.next) / (second.now - second.next) / sigma.now
          return (scale, -scale)
        }
        let denoised2 = sampleScaledAdd(x2, velocity, sigma: second, version: .minimaxH3) { sigma in
          let velocityScale = (second.now - second.next) / (sigma.now - sigma.next)
          return (1, -sigma.now * velocityScale)
        }
        let derivative = sampleScaledAdd(denoised2, clean, sigma: first, version: .minimaxH3) {
          sigma in
          let hMid = log(Double(sigma.now) / Double(sigma.next))
          return (Float(1 / hMid), Float(-1 / hMid))
        }
        let denoisedD = sampleScaledAdd(clean, derivative, sigma: step, version: .minimaxH3) {
          sigma in
          (1, Float(log(Double(sigma.now) / Double(sigma.next)) / 2))
        }
        var result = sampleScaledAdd(sample, denoisedD, sigma: step, version: .minimaxH3) { sigma in
          let ratio = Double(sigma.next) / Double(sigma.now)
          return (Float(ratio * ratio), Float(1 - ratio * ratio + Double(sigma.next) * (ratio - 1)))
        }
        let leftW2 = sampleScaledAdd(leftW, basis[2], sigma: second, version: .minimaxH3) { sigma in
          let ratio = Double(sigma.next) / Double(sigma.now)
          return (
            Float(ratio), Float(((Double(sigma.now) - Double(sigma.next)) * ratio).squareRoot())
          )
        ...
        result = sampleScaledAdd(result, rightW2, sigma: step, version: .minimaxH3) { sigma in
          (
            1,
            Float(
              Double(sigma.next) / Double(sigma.now)
  ...
  func testH3AncestralUpdateWithScaledVelocity() {
    ...
    graph.withNoGrad {
      ...
      for stepSigma: (now: Float, next: Float) in [
        ...
        let noise = graph.variable(like: sample)
        noise.full(newNoise)
        let denoised = sampleScaledAdd(sample, velocity, sigma: stepSigma, version: .minimaxH3) {
          sigma in
          let delta = sigma.now - sigma.next
          let velocityScale = delta > 0 ? videoDelta / delta : 0
          return (1, -sigma.now * velocityScale)
        }.rawValue.toCPU()
        let drift = sampleScaledAdd(sample, velocity, sigma: stepSigma, version: .minimaxH3) {
          sigma in
          let delta = sigma.now - sigma.next
          let velocityScale = delta > 0 ? videoDelta / delta : 0
        ...
        let result = sampleScaledAdd(drift, noise, sigma: stepSigma, version: .minimaxH3) { sigma in
          (1, sigmaUp(sigma))
        }.rawValue.toCPU()
        for frame in 0..<7 {
          for y in 0..<height {
