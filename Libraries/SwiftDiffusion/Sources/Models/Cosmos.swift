...
let CosmosFixedTimeConditionCount = CosmosLayerCount * 9 + 2
let CosmosFixedKVConditionCount = CosmosLayerCount * 2
...
public func CosmosRotaryPositionEmbedding(
  ...
  headDimension: Int = 128, theta: Double = 10_000,
...
private func CosmosAdaLayerNormZero(
  prefix: String, batchSize: Int, hiddenSize: Int, hiddenFeatures: Int, name: String
) -> (ModelWeightMapper, Model) {
  let x = Input()
  ...
  let mapper: ModelWeightMapper = { _ in
    var mapping = ModelWeightMapping()
    mapping["\(prefix).linear_1.weight"] = [linear1.weight.name]
    mapping["\(prefix).linear_2.weight"] = ModelWeightElement(
      [shift.weight.name, scale.weight.name, gate.weight.name],
      offsets: [0, hiddenSize, hiddenSize * 2])
    return mapping
  }
  return (mapper, Model([x, embeddedTimestep, tembShift, tembScale, tembGate], [out, gateOut]))
}

private func CosmosAdaLayerNorm(
  prefix: String, batchSize: Int, hiddenSize: Int, hiddenFeatures: Int, name: String
) -> (ModelWeightMapper, Model) {
  let x = Input()
  let embeddedTimestep = Input()
  ...
  return (mapper, Model([x, embeddedTimestep, tembShift, tembScale], [out]))
}

private func CosmosSelfAttention(
  prefix: String, batchSize: Int, tokenLength: Int, hiddenSize: Int, headDimension: Int,
  numberOfHeads: Int, usesFlashAttention: Bool
) -> (ModelWeightMapper, Model) {
  let x = Input()
  let rot = Input()
  ...
  let mapper: ModelWeightMapper = { _ in
    ...
    mapping["\(prefix).to_q.weight"] = ModelWeightElement(
      [toQueries.weight.name], interleaved: true, numberOfHeads: numberOfHeads,
      headDimension: headDimension)
    mapping["\(prefix).norm_q.weight"] = [normQ.weight.name]
    mapping["\(prefix).to_k.weight"] = ModelWeightElement(
      [toKeys.weight.name], interleaved: true, numberOfHeads: numberOfHeads,
      headDimension: headDimension)
    mapping["\(prefix).norm_k.weight"] = [normK.weight.name]
...
private func CosmosCrossAttention(
  prefix: String, batchSize: Int, tokenLength: Int, contextLength: Int, hiddenSize: Int,
  headDimension: Int, numberOfHeads: Int, usesFlashAttention: Bool
) -> (ModelWeightMapper, Model) {
  let x = Input()
  let keys = Input()
  let values = Input()
  ...
  if usesFlashAttention {
    ...
    out = scaledDotProductAttention(
      attentionScale * queries, attentionScale * attentionKeys, attentionValues)
  } else {
    let query = queries.transposed(1, 2).contiguous()
...
private func CosmosFeedForward(prefix: String, hiddenSize: Int, mlpRatio: Int)
  -> (ModelWeightMapper, Model)
{
  let x = Input()
  ...
  let mapper: ModelWeightMapper = { _ in
    var mapping = ModelWeightMapping()
    mapping["\(prefix).net.0.proj.weight"] = [fc1.weight.name]
    mapping["\(prefix).net.2.weight"] = [fc2.weight.name]
    return mapping
  }
...
private func CosmosTransformerBlock(
  ...
  let (selfAttentionMapper, selfAttention) = CosmosSelfAttention(
    prefix: "\(prefix).attn1", batchSize: batchSize, tokenLength: tokenLength,
  ...
  let (crossAttentionMapper, crossAttention) = CosmosCrossAttention(
    prefix: "\(prefix).attn2", batchSize: batchSize, tokenLength: tokenLength,
  ...
  let (feedForwardMapper, feedForward) = CosmosFeedForward(
    prefix: "\(prefix).ff", hiddenSize: hiddenSize, mlpRatio: mlpRatio)
  out =
    out
...
public func Cosmos(
  batchSize: Int, latentHeight: Int, latentWidth: Int, textLength: Int, inChannels: Int = 16,
  ...
  usesFlashAttention: Bool = true
) -> (ModelWeightMapper, Model) {
  let hiddenSize = numberOfHeads * headDimension
  let patchEmbedInChannels = inChannels + 1
  let patchChannels = patchEmbedInChannels * patchSize.height * patchSize.width
  let projOutChannels = patchSize.height * patchSize.width * outChannels
  let tokenHeight = latentHeight / patchSize.height
  let tokenWidth = latentWidth / patchSize.width
  ...
  let patchEmbed = Dense(count: hiddenSize, noBias: true, name: "x_embedder")
  let zeroMask =
    (0 * hiddenStates.reduced(.mean, axis: [3])).reshaped(
      .NHWC(batchSize, latentHeight, latentWidth, 1))
  let paddedHiddenStates = Functional.concat(axis: 3, hiddenStates, zeroMask)
  var out = paddedHiddenStates.reshaped(
    [
      batchSize, tokenHeight, patchSize.height, tokenWidth, patchSize.width, patchEmbedInChannels,
    ]).permuted(0, 1, 3, 5, 2, 4).contiguous().reshaped(
      .HWC(batchSize, tokenLength, patchChannels))
  out = patchEmbed(out).to(.Float32)
  var blockMappers = [ModelWeightMapper]()
  for i in 0..<layers {
  ...
  out = projOut(out).reshaped(
    [
      ...
      [batchSize, latentHeight, latentWidth, outChannels])
  let mapper: ModelWeightMapper = { format in
    var mapping = ModelWeightMapping()
...
private func CosmosAdaLayerNormZeroFixed(
  prefix: String, timesteps: Int, hiddenSize: Int, hiddenFeatures: Int, name: String
) -> (ModelWeightMapper, Model) {
  let embeddedTimestep = Input()
  let tembShift = Input()
  let tembScale = Input()
...
private func CosmosCrossAttentionFixed(
  prefix: String, batchSize: Int, contextLength: Int, hiddenSize: Int, headDimension: Int,
  numberOfHeads: Int
) -> (ModelWeightMapper, Model) {
  let context = Input()
...
public func CosmosFixed(
  ...
  for i in 0..<layers {
    let (norm1Mapper, norm1) = CosmosAdaLayerNormZeroFixed(
      prefix: "transformer_blocks.\(i).norm1", timesteps: timesteps, hiddenSize: hiddenSize,
    ...
    let (norm2Mapper, norm2) = CosmosAdaLayerNormZeroFixed(
      prefix: "transformer_blocks.\(i).norm2", timesteps: timesteps, hiddenSize: hiddenSize,
    ...
    let (norm3Mapper, norm3) = CosmosAdaLayerNormZeroFixed(
      prefix: "transformer_blocks.\(i).norm3", timesteps: timesteps, hiddenSize: hiddenSize,
  ...
  let (normOutMapper, normOut) = CosmosAdaLayerNormFixed(
    prefix: "norm_out", timesteps: timesteps, hiddenSize: hiddenSize,
  ...
  for i in 0..<layers {
    let (crossAttentionMapper, crossAttention) = CosmosCrossAttentionFixed(
      prefix: "transformer_blocks.\(i).attn2", batchSize: batchSize, contextLength: textLength,
      hiddenSize: hiddenSize, headDimension: headDimension, numberOfHeads: numberOfHeads)
  ...
  let mapper: ModelWeightMapper = { format in
    ...
    mapping["time_embed.t_embedder.linear_2.weight"] = ModelWeightElement(
