...
class NodeTest: XCTestCase {
  ...
  func testHandlesAbsOnImage() {
    do {
      let doc: Document = try SwiftSoup.parse(
        "<p><img src=\"/rez/osi_logo.png\" /></p>", "https://jsoup.org/")
  ...
  func testHandlesAbsPrefixOnHasAttr() {
    do {
      ...
      let doc: Document = try SwiftSoup.parse(
        "<a id=1 href='/foo'>One</a> <a id=2 href='https://jsoup.org/'>Two</a>")
...
*/
  func testHandleAbsOnLocalhostFileUris() {
    do {
      let doc: Document = try SwiftSoup.parse(
        "<a href='password'>One/a><a href='/var/log/messages'>Two</a>", "file://localhost/etc/")
      let one: Element? = try doc.select("a").first()
  ...
  func testHandlesAbsOnProtocolessAbsoluteUris() {
    do {
      let doc1: Document = try SwiftSoup.parse(
        "<a href='//example.net/foo'>One</a>", "http://example.com/")
      let doc2: Document = try SwiftSoup.parse(
        "<a href='//example.net/foo'>One</a>", "https://example.com/")
      ...
      let doc3: Document = try SwiftSoup.parse(
        "<img src=//www.google.com/images/errors/logo_sm.gif alt=Google>", "https://google.com")
      XCTAssertEqual(
        "https://www.google.com/images/errors/logo_sm.gif", try doc3.select("img").attr("abs:src"))
    } catch {
      XCTAssertEqual(1, 2)
    }
  }
  ...
  func testAbsHandlesRelativeQuery() {
    do {
      let doc: Document = try SwiftSoup.parse(
        "<a href='?foo'>One</a> <a href='bar.html?foo'>Two</a>", "https://jsoup.org/path/file?bar")
  ...
  func testAbsHandlesDotFromIndex() {
    do {
      let doc: Document = try SwiftSoup.parse(
        "<a href='./one/two.html'>One</a>", "http://example.com")
      let a1: Element? = try doc.select("a").first()
  ...
  func testUnwrap() {
    do {
      ...
      XCTAssertEqual(
        "<div>One Two <b>Three</b> Four</div>", TextUtil.stripNewlines(try doc.body()!.html()))
  ...
  func testChildNodesCopy() {
    do {
      let doc: Document = try SwiftSoup.parse(
        "<div id=1>Text 1 <p>One</p> Text 2 <p>Two<p>Three</div><div id=2>")
  ...
  func testDeepCloneIndependence() {
    do {
      let doc: Document = try SwiftSoup.parse(
        "<div id=wrap><p class=one data-x=1>Text <span>Child</span></p></div>")
