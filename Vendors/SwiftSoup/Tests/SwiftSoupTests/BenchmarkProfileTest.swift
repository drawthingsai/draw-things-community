...
final class BenchmarkProfileTest: XCTestCase {
  ...
  private func buildBenchmarkHTML(repeatCount: Int) -> String {
    ...
    return "<!doctype html><html><head><title>t</title></head><body>"
      + String(repeating: chunk, count: repeatCount) + "</body></html>"
  }
  ...
  private func buildColossalBenchmarkHTML(repeatCount: Int) -> String {
    let textBlock = String(
      repeating: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. ", count: 20)
    let chunk = """
  ...
  private func buildTitanBenchmarkHTML(repeatCount: Int) -> String {
    let paragraph = String(
      repeating: "Sed ut perspiciatis unde omnis iste natus error sit voluptatem. ", count: 30)
    let chunk = """
  ...
  private func buildOmegaBenchmarkHTML(repeatCount: Int) -> String {
    let paragraph = String(
      repeating: "At vero eos et accusamus et iusto odio dignissimos ducimus. ", count: 40)
    let chunk = """
  ...
  private func buildHyperionBenchmarkHTML(repeatCount: Int) -> String {
    let paragraph = String(
      repeating: "Hyperion lorem ipsum dolor sit amet, consectetur adipiscing elit. ", count: 60)
    let chunk = """
  ...
  private func buildLeviathanBenchmarkHTML(repeatCount: Int) -> String {
    let paragraph = String(
      repeating: "Leviathan lorem ipsum dolor sit amet, consectetur adipiscing elit. ", count: 90)
    let classBlob =
  ...
  private func buildGargantuaBenchmarkHTML(repeatCount: Int) -> String {
    let paragraph = String(
      repeating: "Gargantua lorem ipsum dolor sit amet, consectetur adipiscing elit. ", count: 120)
    let classBlob =
      "alpha beta gamma delta epsilon zeta eta theta iota kappa lambda mu nu xi omicron pi rho sigma tau"
    let chunk = """
  ...
  private func buildBehemothBenchmarkHTML(repeatCount: Int) -> String {
    let paragraph = String(
      repeating: "Behemoth lorem ipsum dolor sit amet, consectetur adipiscing elit. ", count: 160)
    let classBlob =
      "alpha beta gamma delta epsilon zeta eta theta iota kappa lambda mu nu xi omicron pi rho sigma tau"
    let chunk = """
  ...
  private func buildColossusBenchmarkHTML(repeatCount: Int) -> String {
    let paragraph = String(
      repeating: "Colossus lorem ipsum dolor sit amet, consectetur adipiscing elit. ", count: 120)
    let chunk = """
      <div class=\"colossus\">
  ...
  private func buildNebulaBenchmarkHTML(repeatCount: Int) -> String {
    let paragraph = String(
      repeating: "Nebula lorem ipsum dolor sit amet, consectetur adipiscing elit. ", count: 220)
    let classBlob =
      "alpha beta gamma delta epsilon zeta eta theta iota kappa lambda mu nu xi omicron pi rho sigma tau"
    let chunk = """
  ...
  private func buildEntityHeavyBenchmarkHTML(repeatCount: Int) -> String {
    let numericChunk = String(
      repeating: "&#65;&#66;&#67;&#97;&#98;&#99;&#169;&#174;&#8482; ", count: 200)
    let hexChunk = String(
      repeating: "&#x41;&#x42;&#x43;&#x61;&#x62;&#x63;&#xA9;&#xAE;&#x2122; ", count: 200)
    let attrChunk = String(
      repeating: " title=\"&#65;&#66;&#67; &amp; &#x61;&#x62;&#x63;\" ", count: 40)
    let chunk = """
  ...
  private func buildEntityStormBenchmarkHTML(repeatCount: Int) -> String {
    let numericChunk = String(
      repeating: "&#65;&#66;&#67;&#97;&#98;&#99;&#169;&#174;&#8482; ", count: 800)
    let hexChunk = String(
      repeating: "&#x41;&#x42;&#x43;&#x61;&#x62;&#x63;&#xA9;&#xAE;&#x2122; ", count: 800)
    let attrChunk = String(
      repeating:
        " data-n=\"&#65;&#66;&#67;&#97;&#98;&#99;\" data-h=\"&#x41;&#x42;&#x43;&#x61;&#x62;&#x63;\" ",
      count: 80)
    let chunk = """
  ...
  private func buildEntityNamedBenchmarkHTML(repeatCount: Int) -> String {
    let namedChunk = String(
      repeating: "&amp; &lt; &gt; &quot; &apos; &nbsp; &copy; &reg; &trade; ", count: 600)
    let attrChunk = String(
      repeating: " data-x=\"&amp;&amp;&lt;&gt;&quot;&apos;&nbsp;&copy;&reg;&trade;\" ", count: 60)
    let chunk = """
  ...
  private func buildEntityNamedStormBenchmarkHTML(repeatCount: Int) -> String {
    let namedChunk = String(
      repeating:
        "&amp; &lt; &gt; &quot; &apos; &nbsp; &copy; &reg; &trade; &hellip; &mdash; &ndash; &euro; &pound; &yen; &cent; ",
      count: 1200)
    let attrChunk = String(
      repeating:
        " data-x=\"&amp;&lt;&gt;&quot;&apos;&nbsp;&copy;&reg;&trade;&hellip;&mdash;&ndash;&euro;&pound;&yen;&cent;\" ",
      count: 120)
    let chunk = """
  ...
  private func buildEntityNamedExtendedBenchmarkHTML(repeatCount: Int) -> String {
    let extendedChunk = String(
      repeating:
        "&hellip; &mdash; &ndash; &lsquo; &rsquo; &ldquo; &rdquo; &euro; &pound; &yen; &cent; &frac12; &frac14; &frac34; &times; &divide; &micro; &para; &sect; &middot; &laquo; &raquo; ",
      count: 400)
    let attrChunk = String(
      repeating: " data-e=\"&hellip;&mdash;&ndash;&ldquo;&rdquo;&euro;&pound;&yen;&cent;\" ",
      count: 50)
    let chunk = """
  ...
  private func buildAttributeMegaBenchmarkHTML(repeatCount: Int) -> String {
    let attrValue = String(
      repeating: "name=swift-soup&token=abcdef0123456789&v=1.0.0&mode=fast;", count: 80)
  ...
  private func buildDeepNestingBenchmarkHTML(repeatCount: Int, depth: Int, withAttributes: Bool)
    -> String
  {
  ...
  private func buildManabiReaderBenchmarkHTML(repeatCount: Int) -> String {
    let paragraph =
      "This is a paragraph with ruby <ruby>漢字<rt>かんじ</rt></ruby> and <em>emphasis</em>."
    let segment = """
    ...
    return "<!doctype html><html><head><title>manabi-reader</title></head><body>"
      + String(repeating: body, count: repeatCount) + "</body></html>"
  }
  ...
  private func exerciseSelectors(_ doc: Document) throws {
    ...
    if shouldRun("section.hero[data-section=top]") {
      _ = try doc.select("section.hero[data-section=top]")
    }
    if shouldRun("div.alpha:contains(Paragraph)") {
      _ = try doc.select("div.alpha:contains(Paragraph)")
    }
    if shouldRun("p.body:contains(Paragraph)") { _ = try doc.select("p.body:contains(Paragraph)") }
    if shouldRun("section.hero:contains(Heading)") {
      _ = try doc.select("section.hero:contains(Heading)")
    }
    if shouldRun("section:has(form input[type=checkbox])") {
      _ = try doc.select("section:has(form input[type=checkbox])")
    }
  ...
  private func exerciseManabiReaderOps(
    ...
    if let content = try body.getElementById("reader-content") {
      ...
      try trackingFooter.append(
        "<button id='manabi-finished-reading-button'>Finish Reading</button>")
    }
  ...
  func testParseBenchmarkProfile() throws {
    ...
    let benchmarkFilter =
      ProcessInfo.processInfo.environment["SWIFTSOUP_BENCHMARK_SET"]?
      .lowercased()
    ...
    let entityNamedExtendedRepeatCount = scaled(
      envInt("SWIFTSOUP_BENCHMARK_ENTITY_NAMED_EXTENDED_REPEAT", 1))
    let entityNamedStormRepeatCount = scaled(
      envInt("SWIFTSOUP_BENCHMARK_ENTITY_NAMED_STORM_REPEAT", 1))
    ...
    let whitespaceHeavyRepeatCount = scaled(
      envInt("SWIFTSOUP_BENCHMARK_WHITESPACE_HEAVY_REPEAT", 2))
    ...
    let attributeSelectorStressRepeat = envInt(
      "SWIFTSOUP_BENCHMARK_ATTRIBUTE_SELECTOR_STRESS_REPEAT", 4)
    ...
    if includeBenchmark("entity-named-storm"), entityNamedStormRepeatCount > 0 {
      let htmlEntityNamedStorm = buildEntityNamedStormBenchmarkHTML(
        repeatCount: entityNamedStormRepeatCount)
    ...
    if includeBenchmark("entity-named-extended"), entityNamedExtendedRepeatCount > 0 {
      let htmlEntityNamedExtended = buildEntityNamedExtendedBenchmarkHTML(
        repeatCount: entityNamedExtendedRepeatCount)
    ...
    if includeBenchmark("ampersand-heavy"), ampersandHeavyRepeatCount > 0 {
      let htmlAmpersandHeavy = buildAmpersandHeavyBenchmarkHTML(
        repeatCount: ampersandHeavyRepeatCount)
    ...
    if includeBenchmark("attribute-heavy"), attributeHeavyRepeatCount > 0 {
      let htmlAttributeHeavy = buildAttributeHeavyBenchmarkHTML(
        repeatCount: attributeHeavyRepeatCount)
    ...
    if includeBenchmark("attribute-storm"), attributeStormRepeatCount > 0 {
      let htmlAttributeStorm = buildAttributeStormBenchmarkHTML(
        repeatCount: attributeStormRepeatCount)
    ...
    if includeBenchmark("querystring"), queryStringHeavyRepeatCount > 0 {
      let htmlQueryStringHeavy = buildQueryStringHeavyBenchmarkHTML(
        repeatCount: queryStringHeavyRepeatCount)
    ...
    if includeBenchmark("whitespace-heavy"), whitespaceHeavyRepeatCount > 0 {
      let htmlWhitespace = buildWhitespaceHeavyBenchmarkHTML(
        repeatCount: whitespaceHeavyRepeatCount)
    ...
    if includeBenchmark("files"),
      ProcessInfo.processInfo.environment["SWIFTSOUP_BENCHMARK_FILES"] != "0"
    {
      let cwd = FileManager.default.currentDirectoryPath
      let benchmarksURL = URL(fileURLWithPath: cwd).appendingPathComponent("Resources/benchmarks")
      if let files = try? FileManager.default.contentsOfDirectory(
        at: benchmarksURL, includingPropertiesForKeys: nil),
        !files.isEmpty
      {
        var htmlFiles = files.filter { $0.pathExtension.lowercased() == "html" }.sorted {
    ...
    let skipSelectors =
      ProcessInfo.processInfo.environment["SWIFTSOUP_BENCHMARK_SKIP_SELECTORS"] == "1"
    ...
    if iterations > 0 {
      ...
      print(
        "Benchmark elapsed: \(String(format: "%.2f", elapsedMs)) ms over \(iterations) iterations")
    }
