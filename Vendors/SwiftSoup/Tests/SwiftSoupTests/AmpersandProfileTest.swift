...
final class AmpersandProfileTest: XCTestCase {
  func testAmpersandProfile() throws {
    ...
    let chunk = """
      ...
      """
    let repeatCount =
      Int(ProcessInfo.processInfo.environment["SWIFTSOUP_AMPERSAND_PROFILE_REPEAT"] ?? "5") ?? 5
    let iterations =
      Int(ProcessInfo.processInfo.environment["SWIFTSOUP_AMPERSAND_PROFILE_ITERATIONS"] ?? "5") ?? 5
    let html =
      "<!doctype html><html><head><title>ampersand-heavy</title></head><body>"
