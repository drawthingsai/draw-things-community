...
open class Element: Node {
  ...
  public convenience init(
    _ tag: Tag, _ baseUri: String, _ attributes: Attributes, skipChildReserve: Bool = false
  ) {
  ...
  public init(
    _ tag: Tag, _ baseUri: [UInt8], _ attributes: Attributes, skipChildReserve: Bool = false
  ) {
    self._tag = tag
  ...
  open func id() -> String {
    ...
    do {
      return try String(
        decoding: attributes.getIgnoreCaseSlice(key: Element.idString), as: UTF8.self)
    } catch {}
    return ""
  }
  ...
  public func insertChildren(_ index: Int, _ children: [Node]) throws -> Element {
    ...
    try Validate.isTrue(
      val: index >= 0 && index <= currentSize, msg: "Insert position out of bounds.")
  ...
  public func append(_ html: String) throws -> Element {
    let nodes: [Node] = try Parser.parseFragment(html.utf8Array, self, getBaseUriUTF8())
  ...
  public func getElementsByTagNormalized(_ normalizedTagName: [UInt8]) throws -> Elements {
    ...
    let needsTrim =
      (normalizedTagName.first?.isWhitespace ?? false)
      || (normalizedTagName.last?.isWhitespace ?? false)
  ...
  public func getElementsByAttribute(_ key: String) throws -> Elements {
    ...
    func hasAbsPrefix(_ bytes: [UInt8]) -> Bool {
      ...
      return lowerAscii(bytes[0]) == UTF8Arrays.absPrefix[0]
        ...
        && lowerAscii(bytes[2]) == UTF8Arrays.absPrefix[2] && bytes[3] == UTF8Arrays.absPrefix[3]
    }
    let needsTrim =
      (keyBytes.first?.isWhitespace ?? false) || (keyBytes.last?.isWhitespace ?? false)
    ...
    let normalizedKey =
      Attributes.containsAsciiUppercase(trimmedSlice) ? trimmedSlice.lowercased() : trimmedSlice
  ...
  public func getElementsByAttributeNormalized(_ normalizedKey: [UInt8]) -> Elements {
    let needsTrim =
      (normalizedKey.first?.isWhitespace ?? false) || (normalizedKey.last?.isWhitespace ?? false)
  ...
  public func getElementsByAttributeValue(_ key: String, _ value: String) throws -> Elements {
    ...
    let normalizedKey =
      Attributes.containsAsciiUppercase(trimmedKeySlice)
      ? trimmedKeySlice.lowercased() : trimmedKeySlice
    ...
    if isHotKey || (dynamicAttributeValueIndexKeySet?.contains(normalizedKey) ?? false) {
      ...
      let results =
        normalizedAttributeValueIndex?[normalizedKey]?[normalizedValue]?.compactMap { $0.value }
        ?? []
      return Elements(results)
    }
    return try Collector.collect(Evaluator.AttributeWithValue(key, value), self)
  ...
  func getElementsByAttributeValueNormalized(
    ...
    if isHotKey || (dynamicAttributeValueIndexKeySet?.contains(keySlice) ?? false) {
      ...
      let results =
        normalizedAttributeValueIndex?[keySlice]?[valueSlice]?.compactMap { $0.value } ?? []
      return Elements(results)
    }
    return try Collector.collect(Evaluator.AttributeWithValue(key, value), self)
  }
  ...
  /**
     ...
     */
  @inline(__always)
  public func getElementsByAttributeValueStarting(_ key: String, _ valuePrefix: String) throws
    -> Elements
  {
  ...
  /**
     ...
     */
  @inline(__always)
  public func getElementsByAttributeValueEnding(_ key: String, _ valueSuffix: String) throws
    -> Elements
  {
  ...
  /**
     ...
     */
  @inline(__always)
  public func getElementsByAttributeValueContaining(_ key: String, _ match: String) throws
    -> Elements
  {
  ...
  /**
     ...
     */
  public func getElementsByAttributeValueMatching(_ key: String, _ pattern: Pattern) throws
    -> Elements
  {
  ...
  public func getElementsByAttributeValueMatching(_ key: String, _ regex: String) throws -> Elements
  {
    ...
    do {
      ...
      throw Exception.Error(
        type: ExceptionType.IllegalArgumentException, Message: "Pattern syntax error: \(regex)")
    }
  ...
  class TextNodeVisitor: NodeVisitor {
    ...
    public func head(_ node: Node, _ depth: Int) {
      if let textNode = (node as? TextNode) {
        ...
        if !accum.isEmpty && (element.isBlock() || element._tag.getNameUTF8() == UTF8Arrays.br)
  ...
  private func collectTextFast(_ accum: StringBuilder, trimAndNormaliseWhitespace: Bool) {
    ...
    while let node = stack.popLast() {
      ...
      if let element = node as? Element {
        if !accum.isEmpty && (element.isBlock() || Tag.isBr(element._tag)) && !lastWasWhite {
          accum.append(UTF8Arrays.whitespace)
          lastWasWhite = true
        }
      }
  ...
  private func singleTextNoWhitespaceSlice() -> ByteSlice? {
    ...
    for b in slice {
      if StringUtil.isAsciiWhitespaceByte(b) || b == StringUtil.utf8NBSPLead
  ...
  @inline(__always)
  private static func appendNormalisedTextTracking(
  ...
  @inline(__always)
  private static func emitNormalizedSlice(
    _ slice: ArraySlice<UInt8>,
    stripLeading: Bool,
    emittedAny: inout Bool,
    lastWasWhite: inout Bool,
    matcher: inout AsciiKMPMatcher
  ) -> Bool {
  ...
  internal func containsNormalizedTextASCII(_ needleLower: [UInt8]) -> Bool {
    ...
    while let node = stack.popLast() {
      if let textNode = node as? TextNode {
        ...
        if Element.emitNormalizedSlice(
          slice,
  ...
  static func preserveWhitespace(_ node: Node?) -> Bool {
    ...
    if let element = (node as? Element) {
      return element._tag.preserveWhitespace()
        || element.parent() != nil && element.parent()!._tag.preserveWhitespace()
    }
    return false
  }
  ...
  public func hasClass(_ className: [UInt8]) -> Bool {
    ...
    @inline(__always)
    func equalsIgnoreCaseSlice(_ bytes: ByteSlice, _ start: Int, _ length: Int, _ other: [UInt8])
      -> Bool
    {
    ...
    while i < len {
      ...
      if b.isWhitespace {
        if inToken {
          ...
          if tokenLen == wantLen
            && equalsIgnoreCaseSlice(classAttr, tokenStart, tokenLen, className)
          {
            return true
          }
          inToken = false
  ...
  override func outerHtmlHead(_ accum: StringBuilder, _ depth: Int, _ out: OutputSettings) throws {
    if out.prettyPrint()
      && (_tag.formatAsBlock() || (parent() != nil && parent()!.tag().formatAsBlock())
        || out.outline())
    {
      if !accum.isEmpty {
      ...
      if out.prettyPrint()
        && (!childNodes.isEmpty
          && (_tag.formatAsBlock()
            || (out.outline()
              && (childNodes.count > 1
                || (childNodes.count == 1 && !(((childNodes[0] as? TextNode) != nil)))))))
      {
        indent(accum, depth, out)
      }
...
extension Element {
  ...
  func storeSelectorResult(_ query: String, _ result: Elements) {
    ...
    if selectorResultCache == nil {
      ...
      selectorQueryStats = SelectorQueryStats(
        windowSize: Element.selectorResultCacheCapacity
          * Element.selectorResultCacheScanWindowMultiplier
      )
    }
    if !hadCache {
      recordSelectorQuery(query, hit: false)
  ...
  private func recordSelectorQuery(_ query: String, hit: Bool) {
    ...
    let uniqueThreshold =
      (Element.selectorResultCacheCapacity
  ...
  @inline(__always)
  private static func forEachClassNameWithUppercase(
    in bytes: [UInt8], _ visitor: (ArraySlice<UInt8>, Bool) -> Void
  ) {
    var i = 0
    let len = bytes.count
    while i < len {
  ...
  @inline(__always)
  private static func forEachClassNameWithUppercase(
    in bytes: ByteSlice, _ visitor: (ByteSlice, Bool) -> Void
  ) {
    var i = 0
    let len = bytes.count
    while i < len {
  ...
  private func rebuildQueryIndexesCombined(
    ...
    DebugTrace.log(
      "Element.rebuildQueryIndexesCombined: tags=\(needsTags) classes=\(needsClasses) ids=\(needsIds) attrs=\(needsAttributes) hot=\(needsHotAttributes)"
    )
    ...
    if needsHotAttributes {
      hotAttributeIndex.reserveCapacity(
        Element.hotAttributeIndexKeys.count + (dynamicKeys?.count ?? 0))
    }
      ...
      if needsAttributes || needsHotAttributes {
        ...
        if let attrs = element.attributes {
          ...
          for attr in attrs.attributes {
            DebugTrace.log(
              "rebuildQueryIndexesCombined: attr key \(String(decoding: attr.getKeyUTF8(), as: UTF8.self))"
            )
  ...
  func rebuildQueryIndexesForAllTags() {
    ...
    let combinedCount =
      1 + (needsClasses ? 1 : 0) + (needsIds ? 1 : 0) + (needsAttributes ? 1 : 0)
  ...
  func rebuildQueryIndexesForAllClasses() {
    ...
    let needsHotAttributes = isAttributeValueQueryIndexDirty || normalizedAttributeValueIndex == nil
    let combinedCount =
      1 + (needsIds ? 1 : 0) + (needsAttributes ? 1 : 0) + (needsHotAttributes ? 1 : 0)
    if combinedCount > 1 {
  ...
  func rebuildQueryIndexesForAllIds() {
    ...
    let needsHotAttributes = isAttributeValueQueryIndexDirty || normalizedAttributeValueIndex == nil
    let combinedCount =
      1 + (needsClasses ? 1 : 0) + (needsAttributes ? 1 : 0) + (needsHotAttributes ? 1 : 0)
    if combinedCount > 1 {
      rebuildQueryIndexesCombined(
        needsTags: false,
        needsClasses: needsClasses,
  ...
  func rebuildQueryIndexesForAllAttributes() {
    ...
    let needsHotAttributes = isAttributeValueQueryIndexDirty || normalizedAttributeValueIndex == nil
    let combinedCount =
      1 + (needsClasses ? 1 : 0) + (needsIds ? 1 : 0) + (needsHotAttributes ? 1 : 0)
    if combinedCount > 1 {
      rebuildQueryIndexesCombined(
        needsTags: false,
        needsClasses: needsClasses,
  ...
  func rebuildQueryIndexesForHotAttributes() {
    ...
    let needsIds = isIdQueryIndexDirty || normalizedIdIndex == nil
    let needsAttributes = isAttributeQueryIndexDirty || normalizedAttributeNameIndex == nil
    let combinedCount = 1 + (needsClasses ? 1 : 0) + (needsIds ? 1 : 0) + (needsAttributes ? 1 : 0)
    if combinedCount > 1 {
      rebuildQueryIndexesCombined(
        needsTags: false,
        needsClasses: needsClasses,
    ...
    traverseElementsDepthFirst { element in
      if let attrs = element.getAttributes() {
        ...
        for attr in attrs.attributes {
          let keySlice = attr.keySlice
          let key = lowerKeys ? attr.lowerKeySlice() : keySlice
          guard Element.isHotAttributeKey(key) || (dynamicKeys?.contains(key) ?? false) else {
            continue
          }
          let value = attr.lowerTrimmedValueSlice()
