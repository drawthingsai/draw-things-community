...
extension Archive {
  ...
  typealias EndOfCentralDirectoryStructure = (
  ...
  public func addEntry(
    with path: String, relativeTo baseURL: URL,
    ...
    try self.addEntry(
      with: path, fileURL: fileURL, compressionMethod: compressionMethod,
  ...
  /// - Throws: An error if the source file cannot be read or the receiver is not writable.
  public func addEntry(
    with path: String, fileURL: URL, compressionMethod: CompressionMethod = .none,
    bufferSize: Int = defaultWriteChunkSize, progress: Progress? = nil
  ) throws {
    ...
    switch type {
    case .file:
      let entryFileSystemRepresentation = fileManager.fileSystemRepresentation(
        withPath: fileURL.path)
      ...
      try self.addEntry(
        with: path, type: type, uncompressedSize: uncompressedSize,
    ...
    case .symlink:
      provider = { _, _ -> Data in
        ...
        let linkFileSystemRepresentation = fileManager.fileSystemRepresentation(
          withPath: linkDestination)
  ...
  public func addEntry(
    with path: String, type: Entry.EntryType, uncompressedSize: Int64,
    ...
    progress: Progress? = nil, provider: Provider
  ) throws {
    ...
    let (eocdRecord, zip64EOCD) = (
      self.endOfCentralDirectoryRecord, self.zip64EndOfCentralDirectory
    )
    ...
    do {
      ...
      localFileHeader = try self.writeLocalFileHeader(
        path: path, compressionMethod: compressionMethod,
      ...
      _ = try Data.writeLargeChunk(
        existingData, size: existingSize, bufferSize: bufferSize, to: archiveFile)
      let permissions =
        permissions ?? (type == .directory ? defaultDirectoryPermissions : defaultFilePermissions)
      let externalAttributes = FileManager.externalFileAttributesForEntry(
        of: type, permissions: permissions)
      let centralDir = try self.writeCentralDirectoryStructure(
      ...
      let eocd = try self.writeEndOfCentralDirectory(
      ...
      try rollback(
        UInt64(fileHeaderStart), (existingData, existingSize), bufferSize, eocdRecord, zip64EOCD)
  ...
  public func remove(
    _ entry: Entry, bufferSize: Int = defaultReadChunkSize, progress: Progress? = nil
  ) throws {
    guard self.accessMode != .read else { throw ArchiveError.unwritableArchive }
    ...
    for currentEntry in self {
      ...
      if currentEntry != entry {
        ...
        _ = try Data.consumePart(
          of: Int64(currentEntry.localSize), chunkSize: bufferSize,
    ...
    let ecodStructure = try tempArchive.writeEndOfCentralDirectory(
      ...
      operation: .remove)
    (tempArchive.endOfCentralDirectoryRecord, tempArchive.zip64EndOfCentralDirectory) =
      ecodStructure
...
extension Archive {
  ...
  fileprivate func updateOffsetInCentralDirectory(
  ...
  fileprivate func rollback(
    _ localFileHeaderStart: UInt64, _ existingCentralDirectory: (data: Data, size: UInt64),
    ...
    _ = try Data.writeLargeChunk(
      existingCentralDirectory.data, size: existingCentralDirectory.size,
  ...
  fileprivate func makeTempArchive() throws -> (Archive, URL?) {
    var archive: Archive
    var url: URL?
    if self.isMemoryArchive {
      #if swift(>=5.0)
        guard
          let tempArchive = Archive(
            data: Data(), accessMode: .create,
            preferredEncoding: self.preferredEncoding)
        else {
          throw ArchiveError.unwritableArchive
        }
        archive = tempArchive
