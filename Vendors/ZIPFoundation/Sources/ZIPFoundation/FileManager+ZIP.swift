...
extension FileManager {
  ...
  public func zipItem(
    ...
    if isDirectory {
      ...
      if let progress = progress {
        totalUnitCount = subPaths.reduce(
          Int64(0),
          {
      ...
      for entryPath in subPaths {
        ...
        if let progress = progress {
          ...
          try archive.addEntry(
            with: finalEntryPath, relativeTo: finalBaseURL,
      ...
      try archive.addEntry(
        with: sourceURL.lastPathComponent, relativeTo: baseURL,
  ...
  public func unzipItem(
    at sourceURL: URL, to destinationURL: URL, skipCRC32: Bool = false,
    ...
    guard
      let archive = Archive(url: sourceURL, accessMode: .read, preferredEncoding: preferredEncoding)
    else {
    ...
    for entry in sortedEntries {
      ...
      if let progress = progress {
        ...
        crc32 = try archive.extract(
          entry, to: entryURL, skipCRC32: skipCRC32, progress: entryProgress)
      } else {
  ...
  func createParentDirectoryStructure(for url: URL) throws {
    ...
    try self.createDirectory(
      at: parentDirectoryURL, withIntermediateDirectories: true, attributes: nil)
  }
  ...
  class func attributes(from entry: Entry) -> [FileAttributeKey: Any] {
    ...
    let permissions = self.permissions(
      for: externalFileAttributes, osType: osType, entryType: entryType)
    attributes[.posixPermissions] = NSNumber(value: permissions)
    return attributes
  }

  class func permissions(
    for externalFileAttributes: UInt32, osType: Entry.OSType,
  ...
  var fileModificationTime: UInt16 {
    ...
    extension CocoaError {
      public static func error(
        _ code: CocoaError.Code, userInfo: [AnyHashable: Any]? = nil, url: URL? = nil
      ) -> Error {
...
extension URL {
  public func isContained(in parentDirectoryURL: URL) -> Bool {
    ...
    let parentDirectoryURL = URL(fileURLWithPath: parentDirectoryURL.path, isDirectory: true)
      .standardized
